...
export function launchEmulator(rom, button) {
  ...
  (function installPopupBootstrap(w) {
    ...
    const codeN64 = [
	  ...
	  "        \"    <span id=\\\"gp-msg\\\"></span>\" +",
	  "        \"    <button id=\\\"fs-btn\\\" style=\\\"margin-left:auto;background:#222;border:1px solid #333;color:#bbb;padding:4px 8px;border-radius:6px;cursor:pointer;\\\">⤢ Fullscreen</button>\" +", 
	  "        \"  </div>\" +",
	  "        \"  <iframe id=\\\"vnc-frame\\\" src=\\\"\\\" allow=\\\"clipboard-read; clipboard-write; fullscreen\\\" allowfullscreen style=\\\"border:0;flex:1;width:100%;height:100%\\\"></iframe>\" +",  
	  "        \"</div>\" +",
	  "        \"</div>\"",  
	  ...
	  "      // Real fullscreen toggle for the whole popup",
	  "      function toggleFS(){",
	  "        try {",
	  "          if (!document.fullscreenElement) {",
	  "            document.documentElement.requestFullscreen({ navigationUI: 'hide' }).catch(function(e){ console.warn('FS fail', e); });",
	  "          } else {",
	  "            document.exitFullscreen().catch(function(){});",
	  "          }",
	  "        } catch(e){ console.warn('FS toggle error', e); }",
	  "      }",
	  "      var fsBtn = document.getElementById('fs-btn');",
	  "      if (fsBtn) fsBtn.addEventListener('click', toggleFS, { passive:true });",
	  "      document.addEventListener('keydown', function(ev){ if(ev.key==='f' && (ev.ctrlKey||ev.metaKey)){ ev.preventDefault(); toggleFS(); }}, { passive:false });",
	...
	const codeGBA = [
	  ...
	  "        try { f.allowFullscreen = true; } catch(_){ }",
